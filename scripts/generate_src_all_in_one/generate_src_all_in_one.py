...
def amalgamate_one_file(filename, already_included_local_files, already_included_external_files):
    """
    Recursive function that will create an amalgamation for a given header file.
    """
    if filename in already_included_local_files:
        return ""
    already_included_local_files.append(filename)

    separator_line = "//////////////////////////////////////////////////////////////////////////////////////////////////////////////////"
    middle_line = f"//                       {filename}".ljust(len(separator_line) - 2) + "//"
    parsed_result = f"{separator_line}\n{middle_line}\n{separator_line}\n"

    if not os.path.isfile(filename):
        filename = f"{SRC_DIR}/{filename}"
    lines = fread_lines(filename)
    for code_line in lines:
        if is_external_include_line(code_line):
            ...
            if not os.path.exists(SRC_DIR + "/" + new_file):
                print("Argh...")
...
def amalgamate_cpp_files_content(cpp_files: typing.List[str]):
    content = ""
    already_included_local_files = []
    already_included_external_files = []
    for cpp_file in cpp_files:
        content = content + amalgamate_one_file(cpp_file, already_included_local_files, already_included_external_files) + "\n"
    return content
...
def write_amalgamate_cpp_files(folder, dst_file):
    folder = f"{SRC_DIR}/{folder}"
    cpp_files = find_all_files_of_extension(folder, ".cpp")
    content = amalgamate_cpp_files_content(cpp_files)
    content = AUTOGENERATED_HEADER + content
    fwrite_content(dst_file, content)


def auto_generate():
    write_amalgamate_header_file("immvision/immvision.h", f"{ALL_IN_ONE_DIR}/immvision/immvision.h")
    write_amalgamate_cpp_files("immvision", f"{ALL_IN_ONE_DIR}/immvision/immvision.cpp")
