...
void pydef_image(pybind11::module& m)
{
    ...
    auto pyClassImageParams = py::class_<ImageParams>(m, "ImageParams", 
        ...
        m.def("factor_image_params_display_only",
            []()
            {
                return ImmVision::FactorImageParamsDisplayOnly();
            },
            R"DocString(Create ImageParams that display the image only, with no decoration, and no user interaction)DocString"
        );
        
                
        m.def("make_zoom_pan_matrix",
            [](const cv::Point2d & zoomCenter, double zoomRatio, const cv::Size displayedImageSize)
            {
                return ImmVision::MakeZoomPanMatrix(zoomCenter, zoomRatio, displayedImageSize);
            },
            py::arg("zoom_center"),
            py::arg("zoom_ratio"),
            py::arg("displayed_image_size"),
            R"DocString(Create a zoom/pan matrix centered around a given point of interest)DocString"
        );
        ...
        m.def("image",
            ...
            py::arg("mat"),
            py::arg("params"),
            R"DocString(Display an image, with full user control: zoom, pan, watch pixels, etc.
 ...
 - This function requires that both imgui and OpenGL were initialized.
   (for example, use `imgui_runner.run`for Python,  or `HelloImGui::Run` for C++))DocString"
        );
        
                
        m.def("image_display",
            [](const std::string& label_id, const cv::Mat& mat, const cv::Size& imageDisplaySize = cv::Size(), bool refreshImage = false, bool showOptionsButton = false, bool isBgrOrBgra = true)
            {
                return ImmVision::ImageDisplay(label_id, mat, imageDisplaySize, refreshImage, showOptionsButton, isBgrOrBgra);
            },
            py::arg("label_id"),
            py::arg("mat"),
            py::arg("image_display_size") = cv::Size(),
            py::arg("refresh_image") = false,
            py::arg("show_options_button") = false,
            py::arg("is_bgr_or_bgra") = true,
            R"DocString(Only, display the image, with no decoration, and no user interaction (by default)
 ...
 Note: this function requires that both imgui and OpenGL were initialized.
       (for example, use `imgui_runner.run`for Python,  or `HelloImGui::Run` for C++))DocString"
        ...
        m.def("available_colormaps",
            []()
            {
                return ImmVision::AvailableColormaps();
            },
            R"DocString(Return the list of the available color maps
 Taken from https://github.com/yuki-koyama/tinycolormap, thanks to Yuki Koyama)DocString"
        );
        ...
        m.def("clear_texture_cache",
            []()
            {
                return ImmVision::ClearTextureCache();
            },
            R"DocString(Clears the internal texture cache of immvision (this is done automatically at exit time)
 ...
 Note: this function requires that both imgui and OpenGL were initialized.
       (for example, use `imgui_runner.run`for Python,  or `HelloImGui::Run` for C++))DocString"
        ...
        m.def("version_info",
            []()
            {
                return ImmVision::VersionInfo();
            },
            R"DocString(Return immvision version info)DocString"
        );
